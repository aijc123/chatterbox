...
u$2(
                  ...
                  autoBlendYolo.value && u$2("div", { className: "cb-note", style: { color: "#666", fontSize: "0.85em", paddingLeft: "1.4em" }, children: describeLlmGap("autoBlend") ?? "已就绪：触发后会先用 LLM 润色再发。每条触发都会调用一次 LLM（产生 token 消耗）。" }),
u$2("span", { style: { display: "inline-flex", alignItems: "center", gap: ".25em" }, children: [
u$2(
                      "input",
                      {
...
u$2("label", { htmlFor: "autoSendYolo", title: "YOLO：循环里每条非表情消息发出前先送 LLM 润色。配置不全会自动停车。", children: "🤖 YOLO（LLM 润色后再发）" }),
u$2(
                  PromptPicker,
                  {
              ...
              autoSendYolo.value && u$2("div", { className: "cb-note", style: { color: "#666", fontSize: "0.85em", paddingLeft: "1.4em" }, children: describeLlmGap("autoSend") ?? "已就绪：每条非表情消息会用 LLM 润色一次（产生 token 消耗）。" })
            ] })
          ] })
        ]
      }
...
u$2(
          ...
          normalSendYolo.value && u$2("div", { className: "cb-note", style: { color: "#666", fontSize: "0.85em", paddingLeft: "1.4em" }, children: describeLlmGap("normalSend") ?? "已就绪：手动发送的文本会先用 LLM 润色（产生 token 消耗）。" })
        ] })
      ] })
    ] });
  }
...
`;
  function CustomChatSection({ query = "" }) {
    ...
    const visible = !query || "Chatterbox Chat 评论区 WS DOM 主题 CSS 去重 折叠 合并 重复 独轮车 防刷屏 ×N".toLowerCase().includes(query);
    if (!visible) return null;
...
u$2(GroupHeading, { query, children: "LLM" }),
...
u$2("div", { className: "cb-note", style: { color: "#666", fontSize: "0.85em" }, children: "LLM 凭证（API key / 模型 / base URL）复用「发送 → 智能辅助驾驶」面板里那一份。这里只管理 YOLO 用的提示词。" }),
u$2("div", { children: [
