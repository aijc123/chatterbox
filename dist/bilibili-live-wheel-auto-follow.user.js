...
(function (speechToTextWeb) {
  ...
  function I(n2) {
    if (n2.__P && n2.__d) {
      ...
      o2.__v = u2.__v + 1, l$3.vnode && l$3.vnode(o2), q$3(n2.__P, o2, u2, n2.__n, n2.__P.namespaceURI, 32 & u2.__u ? [t2] : null, i2, null == t2 ? $$1(u2) : t2, !!(32 & u2.__u), r2), o2.__v = u2.__v, o2.__.__k[o2.__i] = o2, D$1(i2, o2, r2), u2.__e = u2.__ = null, o2.__e != t2 && P$1(o2);
    }
  }
  function P$1(n2) {
  ...
  function R(u2, t2, i2) {
    var r2, o2, e2, f2;
    t2 == document && (t2 = document.documentElement), l$3.__ && l$3.__(u2, t2), o2 = (r2 = false) ? null : t2.__k, e2 = [], f2 = [], q$3(t2, u2 = t2.__k = k$1(S$1, null, [u2]), o2 || d$2, d$2, t2.namespaceURI, o2 ? null : t2.firstChild ? n$1.call(t2.childNodes) : null, e2, o2 ? o2.__e : t2.firstChild, r2, f2), D$1(e2, u2, f2);
  }
...
"1713546334": {
  ...
  const builtInSources = new Map(Object.entries(DEFAULT_MEME_SOURCES));
  ...
  function validateMemeSource(input) {
    if (typeof input !== "object" || input === null) return null;
    const r2 = input;
    if (typeof r2.roomId !== "number" || !Number.isInteger(r2.roomId) || r2.roomId <= 0) return null;
    const nameRaw = typeof r2.name === "string" ? r2.name.trim() : "";
    if (nameRaw.length === 0) return null;
    const listEndpoint = normalizeSourceUrl(r2.listEndpoint);
    ...
    const randomEndpoint = r2.randomEndpoint == null ? null : normalizeSourceUrl(r2.randomEndpoint);
    if (randomEndpoint) out.randomEndpoint = randomEndpoint;
    const submitPage = r2.submitPage == null ? null : normalizeSourceUrl(r2.submitPage);
    if (submitPage) out.submitPage = submitPage;
    if (Array.isArray(r2.defaultTags)) {
      const cleaned = [];
      for (const t2 of r2.defaultTags) {
    ...
    if (Array.isArray(r2.pauseKeywords)) {
      const cleaned = [];
      for (const t2 of r2.pauseKeywords) {
    ...
    if (r2.keywordToTag && typeof r2.keywordToTag === "object" && !Array.isArray(r2.keywordToTag)) {
      const map = {};
      let count = 0;
      for (const [k2, v2] of Object.entries(r2.keywordToTag)) {
        if (count >= KEYWORD_MAP_MAX_ENTRIES) break;
        if (typeof k2 !== "string" || k2.length === 0 || k2.length > KEYWORD_PATTERN_MAX_LEN) continue;
        if (typeof v2 !== "string") continue;
        const tagName = v2.trim();
  ...
  const userMemeSources = gmSignal("userMemeSources", [], { validate: isUnknownArray });
  j$1(() => {
  ...
  const autoBlendCooldownAuto = gmSignal("autoBlendCooldownAuto", false);
  ...
  const autoBlendAvoidRepeat = gmSignal("autoBlendAvoidRepeat", false);
  ...
  const autoBlendMessageBlacklist = gmSignal("autoBlendMessageBlacklist", {});
  ...
  function normalizeEscapes(input) {
    return input.replace(/\\([0-9a-fA-F]{1,6})\s?|\\([^\n])/g, (_match, hex, char) => {
      if (typeof hex === "string") {
        ...
        if (!Number.isFinite(code) || code === 0 || code >= 55296 && code <= 57343 || code > 1114111) {
          return "�";
        }
  ...
  if (!_GM_getValue(customChatFoldRenameKey, false)) {
    const legacy = _GM_getValue("customChatCardMode");
    if (typeof legacy === "boolean") _GM_setValue("customChatFoldMode", legacy);
    _GM_setValue(customChatFoldRenameKey, true);
  }
  ...
  const llmProvider = gmSignal("hzmLlmProvider", "anthropic", { validate: isValidProvider });
  const llmApiKeyPersist = gmSignal("hzmLlmApiKeyPersist", true);
  const llmApiKey = y$1(
  ...
  const llmModel = gmSignal("hzmLlmModel", "claude-haiku-4-5-20251001");
  const llmBaseURL = gmSignal("hzmLlmBaseURL", "");
  const autoBlendYolo = gmSignal("autoBlendYolo", false);
  const autoSendYolo = gmSignal("autoSendYolo", false);
  const normalSendYolo = gmSignal("normalSendYolo", false);
  ...
  const isStringArray = (v2) => Array.isArray(v2) && v2.every((s2) => typeof s2 === "string");
  const isNonNegativeInt = (v2) => typeof v2 === "number" && Number.isInteger(v2) && v2 >= 0;
  const SEED_KEY = "llmPromptsGlobalSeeded";
  if (!_GM_getValue(SEED_KEY, false)) {
    const existing = _GM_getValue("llmPromptsGlobal");
    if (existing === void 0 || Array.isArray(existing) && existing.length === 0) {
      _GM_setValue("llmPromptsGlobal", [DEFAULT_GLOBAL_PROMPT]);
    }
    _GM_setValue(SEED_KEY, true);
  }
  const llmPromptsGlobal = gmSignal("llmPromptsGlobal", [DEFAULT_GLOBAL_PROMPT], {
    validate: isStringArray
  });
  const llmActivePromptGlobal = gmSignal("llmActivePromptGlobal", 0, { validate: isNonNegativeInt });
  const llmPromptsNormalSend = gmSignal("llmPromptsNormalSend", [], { validate: isStringArray });
  const llmActivePromptNormalSend = gmSignal("llmActivePromptNormalSend", 0, {
    validate: isNonNegativeInt
  });
  const llmPromptsAutoBlend = gmSignal("llmPromptsAutoBlend", [], { validate: isStringArray });
  const llmActivePromptAutoBlend = gmSignal("llmActivePromptAutoBlend", 0, { validate: isNonNegativeInt });
  const llmPromptsAutoSend = gmSignal("llmPromptsAutoSend", [], { validate: isStringArray });
  const llmActivePromptAutoSend = gmSignal("llmActivePromptAutoSend", 0, { validate: isNonNegativeInt });
  ...
  const shadowChipDismissedKeys = gmSignal("shadowChipDismissedKeys", []);
  ...
  const sonioxAudioDeviceId = gmSignal("sonioxAudioDeviceId", "");
  ...
  function enqueueDanmaku(message, roomId, csrfToken, priority = SendPriority.AUTO) {
    return new Promise((resolve, reject) => {
      ...
      if (priority === SendPriority.MANUAL) {
        if (inflight$1 !== null) cancelAutoItem(inflight$1, "preempted");
        for (const q2 of queue) {
          if (q2 !== item) cancelAutoItem(q2, "preempted");
        }
      }
      if (queue.length > MAX_QUEUE_LENGTH) {
        for (let i2 = queue.length - 1; i2 >= 0 && queue.length > MAX_QUEUE_LENGTH; i2--) {
          const q2 = queue[i2];
          if (q2.priority === SendPriority.AUTO && !q2.cancelled) {
            cancelAutoItem(q2, "queue-overflow");
            queue.splice(i2, 1);
          }
        }
      }
  ...
  function cancelPendingAuto() {
    if (inflight$1 !== null) cancelAutoItem(inflight$1, "loop-stopped");
  ...
  const LAPLACE_AUDIT_TIMEOUT_MS = 1e4;
  ...
  function isValidDanmakuNode(node) {
    ...
    return count === 2 || node.classList.contains("chat-colorful-bubble") && node.classList.contains("has-bubble") && count === 4 || node.classList.contains("has-bubble") && count === 3 || node.classList.contains("chat-emoticon") && node.classList.contains("bulge-emoticon");
  }
...
#]# for # a # that #. # with #'# from # by #. The # on # as # is #ing #
  ...
  let visibilityRecoveryHandler = null;
  ...
  const GUARD_ROOM_FETCH_TIMEOUT_MS = 8e3;
  async function fetchGuardRoom(url, init) {
    const controller = new AbortController();
    const t2 = setTimeout(() => controller.abort(), GUARD_ROOM_FETCH_TIMEOUT_MS);
    try {
  ...
  function learnShadowRules(input) {
    ...
    const existingFroms = new Set(
      existingRules.map((r2) => r2.from).filter((s2) => typeof s2 === "string" && s2.length > 0)
    );
    const newRules = [];
    const learnedFroms = [];
    ...
    const manuals = combined.filter((r2) => r2.source !== "learned");
    const learned = combined.filter((r2) => r2.source === "learned");
    let merged;
...
cache = new Map();
    ...
    maxEntries;
    constructor(opts) {
      const m2 = opts?.maxEntries;
      this.maxEntries = typeof m2 === "number" && m2 > 0 ? Math.floor(m2) : DEFAULT_MAX_ENTRIES;
    }
    async get(opts) {
...
_clearForTests() {
  ...
  async function gmFetch(url, init = {}) {
    const { method = "GET", headers, body, timeoutMs = 2e4, signal } = init;
      ...
      return await mergedCache.get({
        ...
        fetcher: async () => {
          ...
          const items = body.items.filter(
            (m2) => m2 != null && typeof m2.content === "string" && m2.content.trim().length > 0
          ).map((m2) => {
  ...
  function isAutoBlendBlacklistedText(text) {
...
["「", "」"],
  ...
  const CPM_MIN_WINDOW_MS = 2e3;
  const COOLDOWN_FLOOR_SEC = 2;
  const COOLDOWN_CEILING_SEC = 60;
  const COOLDOWN_STEALTH_K = 300;
  function pruneOldTimestamps(now) {
    const cutoff = now - CPM_WINDOW_SEC * 1e3;
    let i2 = 0;
    while (i2 < messageTimestamps.length && messageTimestamps[i2] < cutoff) i2++;
    if (i2 > 0) messageTimestamps.splice(0, i2);
  }
  function getCurrentCpm(now) {
    pruneOldTimestamps(now);
    const n2 = messageTimestamps.length;
    if (n2 === 0) return 0;
    const spanMs = now - messageTimestamps[0];
    const windowMs = Math.max(CPM_MIN_WINDOW_MS, Math.min(spanMs, CPM_WINDOW_SEC * 1e3));
    return Math.round(n2 * 6e4 / windowMs);
  }
  function computeAutoCooldownSec(cpm) {
  ...
  function getEffectiveCooldownMs(now) {
    if (!autoBlendCooldownAuto.value) return autoBlendCooldownSec.value * 1e3;
    return computeAutoCooldownSec(getCurrentCpm(now)) * 1e3;
  }
  ...
  function recordDanmaku(rawText, uid, isReply, hasLargeEmote) {
  ...
  function routineTimerTick() {
    ...
    candidates.sort((a2, b2) => b2[1] - a2[1]);
  ...
  function wheelFoldKey(value) {
    const compacted = compactText(value).replace(/(.)\1+/g, "$1").toLowerCase();
    return collapseRepeatedString(compacted);
  }
  function collapseRepeatedString(s2) {
    const n2 = s2.length;
    if (n2 < 2) return s2;
    for (let k2 = 1; k2 * 2 <= n2; k2++) {
      if (n2 % k2 !== 0) continue;
      const unit = s2.slice(0, k2);
      let matches = true;
      for (let i2 = k2; i2 < n2; i2 += k2) {
        if (s2.slice(i2, i2 + k2) !== unit) {
          matches = false;
          break;
        }
      }
  ...
  function usefulBadgeText(raw, uname) {
    ...
    const text = level === null ? compactText(raw).replace(/^粉丝牌[:：]?/u, "").replace(/^荣耀[:：]?/u, "").replace(/^用户等级[:：]?/u, "") : formatBadgeLevel(level);
  ...
  function trimRenderQueue(queue2) {
    if (queue2.length > CUSTOM_CHAT_MAX_MESSAGES) {
      queue2.splice(0, queue2.length - CUSTOM_CHAT_MAX_MESSAGES);
    }
  }
...
#${ROOT_ID$1} .lc-chat-message:focus-within .lc-chat-actions {
  opacity: 1;
...
u$2(
  ...
  const STYLE_ID$2 = "laplace-custom-chat-style";
  ...
  const messageByCardKey = new Map();
  const recentCardKeys = new Map();
  ...
  function gcRecentCardKeys(now) {
    for (const [key, ts] of recentCardKeys) {
      if (now - ts > 9e3) recentCardKeys.delete(key);
    }
  }
  ...
  function updateUnread() {
    ...
    if (unreadBtn) {
      if (isFollowing()) {
        ...
        unreadBtn.textContent = unread > 0 ? `${formatUnread(unread)} 条新消息，点击回到底部` : followMode === "frozenByButton" ? "已手动暂停跟随" : "正在浏览历史";
  ...
  function addEvent(event) {
    ...
    if (customChatFoldMode.value && event.kind === "danmaku") {
      ...
      if (existing && lastSeen !== void 0 && now - lastSeen <= 9e3) {
  ...
  const MARKER$1 = "lc-dm-direct";
  const STYLE_ID$1 = "lc-dm-direct-style";
  const STYLE$1 = `
...
.chat-item.danmaku-item:hover .${MARKER$1},
.${MARKER$1}:hover {
  opacity: 1;
  pointer-events: auto;
...
.${MARKER$1} button:hover {
  background: rgba(0, 0, 0, .82);
}
html.lc-dm-direct-always .${MARKER$1} {
  opacity: 1;
  pointer-events: auto;
}
`;
  function injectButtons(node, msg, uid, uname) {
    if (node.querySelector(`.${MARKER$1}`)) return;
    const anchor = node.querySelector(".danmaku-item-right");
    if (!anchor) return;
    const container = document.createElement("span");
    container.className = MARKER$1;
  ...
  function handleDelegatedClick(e2) {
    ...
    const btn = target.closest(`.${MARKER$1} button`);
    if (!btn) return;
    e2.stopPropagation();
    const container = btn.closest(`.${MARKER$1}`);
  ...
  function stripHandoffParams(url) {
    try {
...
`;
  ...
  function ensureBadge(node, count) {
    let badge = node.querySelector(`.${MARKER}`);
    if (!badge) {
    ...
    badge.setAttribute("aria-label", `近 ${FOLD_WINDOW_MS / 1e3} 秒内同一弹幕共出现 ${count} 次`);
  }
...
emitExisting: false
  ...
  function stopNativeChatFold() {
    ...
    for (const badge of Array.from(document.querySelectorAll(`.${MARKER}`))) {
      badge.remove();
    }
    foldByKey.clear();
  }
...
distinct_uid_count: Math.min(distinct, b2.msgCount)
  ...
  const EXTERNAL_SERVICES = [
    {
      ...
      description: "使用你浏览器当前的 B 站登录会话访问直播相关接口，并直连直播 WebSocket。用于发送弹幕、读取房间号 / 表情包 / 粉丝牌房间、巡检限制状态，发送后等 WS / DOM 回显以判断是否真的广播（影子屏蔽校验），以及为 Chatterbox Chat 提供弹幕 / 礼物 / 醒目留言 / 进场等事件源。",
      status: () => "always"
    },
    {
      name: "AI 弹幕审核",
      ...
      description: "当弹幕发送失败或疑似被屏蔽且开启了 AI 规避功能后，脚本会将弹幕文本发送至此服务进行敏感词检测，并尝试自动改写后重新发送。",
    ...
    },
    {
      ...
      description: "烂梗面板优先从 chatterbox-cloud 自建后端拉取聚合数据，后端不可用时降级到本地直拉 LAPLACE 烂梗列表。复制烂梗时会向服务报告使用次数。",
      status: () => "always"
    },
    {
      ...
      description: "本仓库 server/ 自建后端，聚合 LAPLACE + SBHZM + 社区贡献的梗库。可在设置里通过 cbBackendUrlOverride 指向自有部署或本地 localhost 实例。",
    ...
    },
    {
      ...
      description: "完全可选。开启后只同步巡检摘要、选定的影子屏蔽规则或 live-desk 心跳，不会上传 cookie、csrf、localStorage 或完整 B 站接口响应；HTTPS-only（loopback 除外）。也可由保安室通过 URL 查询参数接管直播页（如 dry-run 模式）。",
    ...
    },
    {
      ...
      description: "默认关闭，必须自己填 API Key 才会调用。支持 Anthropic、OpenAI，以及任何 OpenAI 兼容自填 base URL（DeepSeek、Moonshot、OpenRouter、Ollama、小米 mimo 等）；自定义域走脚本管理器的 @connect 兜底，每个新域首次访问仍会单独弹窗确认。Prompt 内容仅包含当前要改写的弹幕或候选梗及必要上下文，不会带 cookie、csrf 或其他私人数据。",
      status: () => {
        if (llmApiKey.value.trim() === "") return "off";
        const anyConsumer = normalSendYolo.value || autoBlendYolo.value || autoSendYolo.value || hzmDriveMode.value === "llm";
  ...
  function AboutTab() {
    const initialSeenRef = A$1(lastSeenVersion.value);
...
u$2("span", { children: "作者: NougatDev" }),
...
u$2("div", { className: "cb-note", style: { color: "#666", marginBottom: ".75em" }, children: "大部分配置只保存在你的浏览器里，由脚本管理器本地存储；下面也列出脚本运行时可能与之通信的外部服务，请按需启用。" }),
u$2("div", { style: { marginBottom: "1em" }, children: [
u$2("div", { style: { fontWeight: "bold", marginBottom: ".25em", fontSize: ".95em" }, children: "本地保存的数据" }),
u$2("ul", { style: { margin: 0, paddingLeft: "1.25em", fontSize: ".9em", color: "#555" }, children: LOCAL_DATA_ITEMS.map((item) => u$2("li", { style: { marginBottom: ".15em" }, children: item }, item)) })
        ] }),
u$2("div", { style: { fontWeight: "bold", marginBottom: ".5em", fontSize: ".95em" }, children: "可能访问的外部服务" }),
u$2("div", { className: "cb-list", style: { display: "flex", flexDirection: "column", gap: ".75em" }, children: EXTERNAL_SERVICES.map((service) => {
...
u$2(
                  "div",
                  {
                    ...
                    children: [
u$2("div", { style: { fontWeight: "bold" }, children: service.url ? u$2("a", { href: service.url, target: "_blank", rel: "noopener", style: LINK_STYLE, children: service.name }) : service.name }),
u$2(
                        "span",
                        {
                          title: badge.title,
...
u$2("div", { className: "cb-note", style: { color: "#a15c00", marginTop: ".75em", fontSize: ".85em" }, children: "反馈问题或截图时，请不要公开 cookie、csrf token、账号密钥、localStorage dump、私人房间规则或私有同步地址。" })
      ] })
    ] });
  }
  ...
  function PromptPicker({
    ...
    return u$2(
      NativeSelect,
      {
        ...
        onChange: (e2) => {
          const v2 = Number.parseInt(e2.currentTarget.value, 10);
          if (!Number.isNaN(v2)) onActiveIndexChange(v2);
        },
        children: isEmpty ? u$2("option", { value: "", children: emptyText ?? "(空)" }) : prompts.map((p2, i2) => u$2("option", { value: String(i2), children: [
          i2 + 1,
          ": ",
          getPromptPreview(p2, previewGraphemes)
        ] }, i2))
      }
    );
  }
  function YoloCallout({
    feature,
    enabled,
    readyText
  }) {
    ...
    if (!gap) {
      return u$2("div", { className: "cb-note", style: { paddingLeft: "1.4em" }, children: readyText });
    }
    return u$2("div", { className: "cb-note", style: { paddingLeft: "1.4em" }, children: [
      gap,
u$2(
        "button",
        {
...
u$2(
        "input",
        {
          ...
          disabled,
          onInput: (e2) => {
            let v2 = Number.parseInt(e2.currentTarget.value, 10);
...
u$2("span", { className: "cb-soft", "aria-hidden": "true", style: { fontSize: "10px", whiteSpace: "nowrap" }, children: rangeText })
  ...
  function modeButtonStyle$2(active) {
    return {
...
u$2("div", { style: { margin: ".5em 0", display: "grid", gap: ".35em" }, children: [
u$2("div", { className: "cb-note", children: '黑名单中的弹幕(精确匹配,trim 后)永远不会触发自动跟车。适合屏蔽 "666"、"+1"、"哈哈哈" 这类无意义高频水弹幕。' }),
        entries.length > 0 ? u$2("div", { style: { maxHeight: "150px", overflowY: "auto", display: "grid", gap: ".25em" }, children: entries.map((text) => u$2(
          "div",
          {
            style: {
              display: "flex",
...
u$2(
          ...
          text
        )) }) : u$2("div", { className: "cb-empty", children: "暂无文本黑名单" }),
u$2("div", { style: { display: "flex", gap: ".35em", alignItems: "center", flexWrap: "wrap" }, children: [
u$2(
            "input",
            {
              ...
              value: addText.value,
              onInput: (e2) => {
                addText.value = e2.currentTarget.value;
              },
              onKeyDown: (e2) => {
                if (e2.key === "Enter" && !e2.isComposing) {
                  e2.preventDefault();
...
u$2("button", { type: "button", className: isOn ? "cb-danger" : "cb-primary", onClick: () => void toggleEnabled(), children: isOn ? "停止跟车" : "开始跟车" }),
u$2(
                "span",
                {
                  style: {
...
u$2("div", { className: "cb-segment", children: [
                ["safe", "normal", "hot"].map((preset) => u$2(
                  "button",
                  {
                    ...
                    style: modeButtonStyle$2(currentPreset === preset),
...
u$2(
                  "button",
                  {
                    ...
                    style: modeButtonStyle$2(currentPreset === "custom"),
...
u$2("label", { htmlFor: "autoBlendUseReplacements", children: "套用替换规则" })
                  ] }),
u$2("span", { style: { display: "inline-flex", alignItems: "center", gap: ".25em", flexWrap: "wrap" }, children: [
u$2(
                      "input",
                      {
                        id: "autoBlendYolo",
                        ...
                        onInput: (e2) => {
                          autoBlendYolo.value = e2.currentTarget.checked;
                        }
                      }
                    ),
u$2(
...
u$2(
                      PromptPicker,
                      {
                        ...
                        onActiveIndexChange: (i2) => {
                          llmActivePromptAutoBlend.value = i2;
                        },
                        previewGraphemes: 12,
...
u$2(
                      "input",
                      {
                        ...
                        onInput: (e2) => {
                          markCustom();
                          autoBlendAvoidRepeat.value = e2.currentTarget.checked;
                        }
                      }
                    ),
u$2(
...
u$2(
                      "input",
                      {
                        ...
                        onInput: (e2) => {
                          markCustom();
                          autoBlendCooldownAuto.value = e2.currentTarget.checked;
                        }
                      }
                    ),
u$2(
...
u$2("span", { style: { color: "#a15c00" }, title: "更激进：命中一波后会连发多条达标弹幕，更容易被风控。", children: "（更激进）" })
                ...
                autoBlendSendCount.value * msgSendInterval.value > autoBlendCooldownSec.value && u$2(
                  "div",
                  {
                    role: "alert",
                    ...
                    children: [
                      ...
                      "s ——开启时会再次确认；建议把「每次发X遍」调小或把冷却调大。"
                    ]
                  }
                ),
u$2(BlacklistPanel, {}),
u$2(MessageBlacklistPanel, {})
              ]
            }
          )
        ]
...
u$2(
                "select",
                {
                  ...
                  value: String(idx),
                  onChange: (e2) => {
                    activeTemplateIndex.value = Number.parseInt(e2.currentTarget.value, 10);
                  },
...
u$2("label", { htmlFor: "randomColor", children: "随机颜色" })
              ] }),
u$2("span", { className: "cb-row", children: [
u$2(
                  "input",
...
u$2("label", { htmlFor: "randomInterval", children: "间隔增加随机性" })
              ] }),
u$2("span", { className: "cb-row", children: [
u$2(
                  "input",
...
u$2("label", { htmlFor: "randomChar", children: "随机字符" })
              ] }),
u$2("span", { className: "cb-row", children: [
u$2(
                  "input",
...
u$2("label", { htmlFor: "persistSendState", children: "保持当前直播间独轮车开关状态" })
              ] }),
u$2("span", { className: "cb-row", style: { flexWrap: "wrap", gap: ".25em" }, children: [
u$2(
                  "input",
                  {
                    id: "autoSendYolo",
                    ...
                    onInput: (e2) => {
                      autoSendYolo.value = e2.currentTarget.checked;
                    }
                  }
                ),
u$2(
...
u$2(
                  PromptPicker,
                  {
                    ...
                    onActiveIndexChange: (i2) => {
                      llmActivePromptAutoSend.value = i2;
                    },
                    previewGraphemes: 12,
                    className: "lc-min-w-[120px] lc-max-w-[180px] lc-truncate",
  ...
  async function pickByLLM(roomId, source, opts) {
    ...
    try {
      const chooser = opts?.chooser ?? (await __vitePreload(async () => {
        const { chooseMemeWithLLM: chooseMemeWithLLM2 } = await Promise.resolve().then(() => llmDriver);
      ...
      const chosenContent = await chooser({
        ...
        apiKey,
        model: llmModel.value,
        baseURL: llmBaseURL.value.trim() || void 0,
...
"openai-compat": "OpenAI 兼容"
  ...
  function validateLlmBaseUrl(raw) {
    if (!/^https?:\/\//i.test(raw)) {
  ...
  function LlmApiConfigPanel({ showTestConnection = true }) {
    ...
    const handleTestLLM = async () => {
      ...
      testError.value = "";
      try {
        const { testLLMConnection: testLLMConnection2 } = await __vitePreload(async () => {
          const { testLLMConnection: testLLMConnection3 } = await Promise.resolve().then(() => llmDriver);
          return { testLLMConnection: testLLMConnection3 };
        }, true ? void 0 : void 0);
        const r2 = await testLLMConnection2({
          provider: llmProvider.value,
          apiKey: llmApiKey.value,
          model: llmModel.value,
          baseURL: llmBaseURL.value.trim() || void 0
    ...
    const inputStyle = { boxSizing: "border-box", width: "100%" };
    return u$2("div", { className: "cb-stack", style: { margin: ".5em 0", gap: "10px" }, children: [
u$2("div", { style: STACK_STYLE, children: [
u$2("span", { style: FIELD_LABEL_STYLE, children: "Provider" }),
u$2(
          "div",
          {
...
gridTemplateColumns: "repeat(auto-fit, minmax(94px, 1fr))",
            ...
            children: ["anthropic", "openai", "openai-compat"].map((p2) => u$2(
              "button",
              {
                type: "button",
                "aria-pressed": llmProvider.value === p2,
                style: modeButtonStyle$1(llmProvider.value === p2),
                title: PROVIDER_TITLE[p2],
                onClick: () => {
                  llmProvider.value = p2;
...
u$2("div", { style: STACK_STYLE, children: [
u$2("span", { style: FIELD_LABEL_STYLE, children: "API Key" }),
u$2(
          "input",
          {
            type: "password",
            ...
            onInput: (e2) => {
              llmApiKey.value = e2.currentTarget.value;
              testStatus.value = "idle";
            },
...
u$2("div", { style: { display: "flex", alignItems: "center", gap: "6px", flexWrap: "wrap" }, children: [
u$2("span", { style: FIELD_HINT_STYLE, children: apiKeyConfigured ? `已配置：${maskKey(llmApiKey.value)}` : "未配置" }),
u$2(
            "button",
            {
              type: "button",
...
u$2(
                "input",
                {
                  ...
                  onInput: (e2) => {
                    llmApiKeyPersist.value = e2.currentTarget.checked;
                  }
                }
              ),
u$2("span", { title: "不勾：key 仅留在内存，刷新页面就清空，GM 存储里的旧值也立即抹掉", children: "保存到 GM 存储（关闭后仅本次会话有效）" })
            ]
          }
        )
      ] }),
u$2("div", { style: STACK_STYLE, children: [
u$2("span", { style: FIELD_LABEL_STYLE, children: "模型" }),
u$2(
          "input",
          {
            type: "text",
            ...
            onInput: (e2) => {
              llmModel.value = e2.currentTarget.value;
              testStatus.value = "idle";
            },
      ...
      llmProvider.value === "openai-compat" && u$2("div", { style: STACK_STYLE, children: [
u$2("span", { style: FIELD_LABEL_STYLE, children: "Base URL" }),
u$2(
          "input",
          {
            type: "url",
            ...
            onInput: (e2) => {
              llmBaseURL.value = e2.currentTarget.value;
              testStatus.value = "idle";
            },
...
u$2("span", { style: FIELD_HINT_STYLE, children: "带不带 /v1 都行，自动补全到 /v1/chat/completions。" }),
        (() => {
          const v2 = llmBaseURL.value.trim();
          if (v2 === "") return null;
          const warn = validateLlmBaseUrl(v2);
          if (!warn) return null;
          return u$2("span", { role: "status", "aria-live": "polite", style: { color: "#a15c00", fontSize: "11px" }, children: [
            "⚠️ ",
            warn
          ] });
        })()
      ] }),
      showTestConnection && u$2("div", { style: { display: "flex", alignItems: "center", gap: "6px", flexWrap: "wrap" }, children: [
u$2(
          "button",
          {
            type: "button",
        ...
        testStatus.value === "fail" && u$2("span", { style: { color: "#c00", fontSize: "11px", wordBreak: "break-all", flex: "1 1 100%" }, children: [
          "连接失败：",
          testError.value
        ] })
      ] }),
u$2("div", { className: "cb-note", style: { color: "#a15c00" }, children: [
        llmApiKeyPersist.value ? "Key 明文保存在浏览器 GM 存储；共用电脑或备份导出会暴露。担心可关掉「保存到 GM 存储」改为仅本会话。" : "Key 仅留在内存，刷新页面后清空。",
        "openai-compat 自定义域首次调用时 Tampermonkey 会弹权限确认，需手动允许。"
      ] })
    ] });
  }
  function LlmApiConfigSummary({ onJumpToSettings }) {
    const apiKeyConfigured = llmApiKey.value.trim().length > 0;
    const baseLabel = llmProvider.value === "openai-compat" ? llmBaseURL.value.trim() || "未填 base URL" : "";
    return u$2("div", { className: "cb-panel", style: { display: "grid", gap: "4px" }, children: [
u$2("div", { style: { display: "flex", alignItems: "baseline", gap: "6px", flexWrap: "wrap" }, children: [
u$2("strong", { children: "LLM" }),
        apiKeyConfigured ? u$2("span", { className: "cb-soft", style: { color: "#0a7f55" }, children: "已配置" }) : u$2("span", { style: { color: "#c00" }, children: "未配置" }),
        onJumpToSettings && u$2(
          "button",
          {
            type: "button",
            style: {
      ...
      apiKeyConfigured && u$2("div", { className: "cb-soft", style: { wordBreak: "break-all", fontSize: "11px" }, children: [
        PROVIDER_TITLE[llmProvider.value].split("（")[0],
        " · ",
        maskKey(llmApiKey.value),
        " ·",
        " ",
        llmModel.value || "未填模型",
        baseLabel && ` · ${baseLabel}`
...
u$2("span", { children: [
            ...
            isOn && u$2("span", { className: "cb-soft", children: "运行中" })
          ] }),
u$2("div", { className: "cb-body cb-stack", children: [
u$2("div", { className: "cb-note", style: { marginBottom: ".25em" }, children: "条件满足时，会以你的账号自动从烂梗库挑选并发送弹幕。第一次开启建议先打开下方的「试运行」观察效果。" }),
u$2("div", { style: { display: "grid", gridTemplateColumns: "1fr auto", gap: ".5em", alignItems: "center" }, children: [
u$2("button", { type: "button", className: isOn ? "cb-danger" : "cb-primary", onClick: () => void toggleEnabled(), children: isOn ? "停车" : "开车" }),
u$2(
                "span",
                {
                  style: {
...
u$2("label", { htmlFor: "hzmDryRun", title: "开启后只在日志显示候选，不真发到弹幕——新手强烈建议先开", children: "试运行（只观察，不发送）" }),
...
u$2("b", { children: stats.llmCalls }),
            ...
            sendMsg.value && u$2("div", { role: "alert", style: { color: "#ff3b30", fontSize: "12px", fontWeight: 650, lineHeight: 1.5 }, children: "⚠️ 文字独轮车正在运行 —— 与智驾叠加会同时往 B 站发，极易超出限速被风控。开车前会再次确认；建议先停一边。" }),
            mode === "llm" && llmApiKey.value.trim() === "" && u$2("div", { role: "alert", style: { color: "#ff3b30", fontSize: "12px", fontWeight: 650, lineHeight: 1.5 }, children: "⚠️ LLM 智驾未配置 API key。开车前请先到「设置 → LLM」填好 key，否则无法工作。" })
          ] }),
u$2(
            "details",
            {
...
u$2(
          ...
          mode === "llm" && u$2("div", { style: { marginTop: ".5em" }, children: u$2(
            LlmApiConfigSummary,
            {
              onJumpToSettings: () => {
                activeTab.value = "settings";
...
u$2("div", { className: "cb-body", children: [
u$2(
              "div",
              {
                className: "cb-row",
...
u$2(
                  ...
                  copiedFlash.value && u$2("span", { role: "status", "aria-live": "polite", style: { color: "#168a45", fontSize: "11px", fontWeight: 650 }, children: "✓ 已复制" }),
u$2("span", { className: "cb-soft", style: { marginLeft: "auto", fontSize: "11px" }, children: [
...
u$2(
  ...
  async function fetchRandomFallback(source) {
    ...
    return all.filter((x2) => x2?.content != null && x2.content !== "");
  }
  ...
  async function refreshTrendingMemes(force = false) {
    const now = Date.now();
    if (!force && now - lastFetchAt < TTL_MS) return void 0;
...
u$2(
  ...
  function MemesList() {
    ...
    const prevRectsRef = A$1( new Map());
...
u$2("label", { htmlFor: "enableMemeContribution", style: { fontSize: "12px" }, children: "自动挖掘待贡献梗" })
        ] }),
...
u$2(
                    ...
                    memeSource && u$2(
                      "button",
                      {
                        ...
                        title: isCbOpen ? `当前正在贡献到 chatterbox-cloud。点击改为上传到 ${memeSource.name}（一次只能开一个表单）` : otherFormOpenForAnotherText ? "另一条候选的表单正打开；点这里会切换到本条" : `选标签后上传到 ${memeSource.name}（API：POST /api/admin/memes）`,
                        onClick: () => {
                    ...
                    cbBackendEnabled.value && u$2(
                      "button",
                      {
                        ...
                        title: isSbhzmOpen ? "当前正在上传到 SBHZM。点击改为贡献到 chatterbox-cloud（一次只能开一个表单）" : otherFormOpenForAnotherText ? "另一条候选的表单正打开；点这里会切换到本条" : "选标签后提交到 chatterbox-cloud(进 pending 队列等管理员审核)",
                        onClick: () => {
...
u$2(
              "input",
              {
                ...
                onInput: (e2) => {
                  normalSendYolo.value = e2.currentTarget.checked;
                }
              }
            ),
u$2(
...
u$2(
              PromptPicker,
              {
                ...
                onActiveIndexChange: (i2) => {
                  llmActivePromptNormalSend.value = i2;
                },
                previewGraphemes: 12,
                className: "lc-min-w-[120px] lc-max-w-[180px] lc-truncate",
...
opacity: meta.isLocked && !isCopied ? 0.5 : 1
  ...
  function PromptManager({
    ...
    return u$2(S$1, { children: [
u$2("div", { class: "lc-flex lc-gap-1 lc-items-center lc-flex-wrap lc-mb-2", children: [
u$2(
          PromptPicker,
          {
            id: selectId,
...
u$2(Button, { variant: "outline", size: "sm", onClick: addPrompt, children: "新增" }),
u$2(
          Button,
          {
            variant: "outline",
...
u$2(
        ...
        {
          value: currentPrompt,
          onInput: (e2) => updateActivePrompt(e2.currentTarget.value),
...
"maxLogLines"
  ...
  function backupValueEqual(a2, b2) {
    if (a2 === b2) return true;
    if (typeof a2 !== typeof b2) return false;
    if (a2 === null || b2 === null) return false;
    if (Array.isArray(a2) || Array.isArray(b2)) {
      if (!Array.isArray(a2) || !Array.isArray(b2) || a2.length !== b2.length) return false;
      for (let i2 = 0; i2 < a2.length; i2++) {
        if (!backupValueEqual(a2[i2], b2[i2])) return false;
      }
      return true;
    }
  ...
  function shortRepr(val, maxLen = 48) {
    if (val === void 0) return "（未设置）";
    let s2;
    try {
      s2 = typeof val === "string" ? val : JSON.stringify(val);
    } catch {
      s2 = String(val);
    }
    if (s2.length > maxLen) s2 = `${s2.slice(0, maxLen - 1)}…`;
    return s2;
  }
  function previewImportSettings(json) {
    let data;
    try {
  ...
  function matchesSearchQuery(haystackKeywords, query) {
...
u$2("div", { style: { display: "flex", gap: ".5em", flexWrap: "wrap" }, children: [
u$2("button", { className: "cb-btn", onClick: handlePreview, disabled: !importText.value.trim(), type: "button", children: "预览变更" }),
u$2(
              "button",
              {
                className: "cb-btn",
          ...
          preview.value?.ok && u$2(
            "section",
            {
              ...
              children: [
u$2("div", { style: { fontWeight: 650, marginBottom: ".25em" }, children: [
                  "即将覆盖 ",
                ...
                preview.value.changes.length === 0 ? u$2("div", { className: "cb-note", style: { color: "#666" }, children: "没有字段会被覆盖（备份内容与当前设置一致）。" }) : u$2("ul", { style: { margin: 0, paddingLeft: "1.1em", maxHeight: "160px", overflowY: "auto" }, children: preview.value.changes.map((c2) => u$2("li", { style: { marginBottom: "2px" }, children: [
u$2("code", { style: { fontFamily: "monospace", fontSize: "0.95em" }, children: c2.key }),
u$2("div", { className: "cb-note", style: { fontSize: "0.9em" }, children: [
u$2("span", { style: { color: "#a15c00" }, children: [
                      "当前 ",
                      c2.before
                    ] }),
u$2("span", { style: { margin: "0 .35em" }, children: "→" }),
u$2("span", { style: { color: "#168a45" }, children: [
                      "导入后 ",
                      c2.after
                    ] })
                  ] })
                ] }, c2.key)) }),
                preview.value.skipped.length > 0 && u$2("div", { className: "cb-note", style: { marginTop: ".25em", color: "#a15c00" }, children: [
                  "跳过（格式不匹配）：",
                  preview.value.skipped.join("、")
                ] })
              ]
            }
          ),
...
u$2(
          ...
          (() => {
            const v2 = cbBackendUrlOverride.value.trim();
            if (v2 === "") return null;
            const warn = validateBackendUrl(v2);
            if (!warn) return null;
            return u$2("span", { role: "status", "aria-live": "polite", style: { color: "#a15c00", fontSize: "0.8em" }, children: [
              "⚠️ ",
              warn
            ] });
          })()
...
u$2(
                      ...
                      (() => {
                        ...
                        const r2 = sanitizeCustomChatCss(draft);
                        const issues = [];
                        if (r2.truncated) {
                          issues.push(
                            `已截断到 ${formatBytes(CUSTOM_CHAT_CSS_MAX_LENGTH)}（原文 ${formatBytes(r2.originalLength)}）`
                          );
                        }
                        if (r2.removedImports > 0) issues.push(`剔除 ${r2.removedImports} 条 @import`);
                        if (r2.removedUrlSchemes > 0) issues.push(`中和 ${r2.removedUrlSchemes} 条不安全 url()`);
                        if (r2.removedLegacyHooks > 0) issues.push(`移除 ${r2.removedLegacyHooks} 条 expression/behavior`);
                        if (issues.length === 0) {
                          return u$2("span", { className: "cb-note", style: { fontSize: "0.8em", color: "#6e6e73" }, children: [
                            "当前大小 ",
                            formatBytes(r2.originalLength),
                            " / 上限 ",
                            formatBytes(CUSTOM_CHAT_CSS_MAX_LENGTH)
                          ] });
                        }
                        return u$2(
                          "span",
...
u$2(
                      "input",
                      {
                        ...
                        disabled: !customChatEnabled.value,
                        onInput: (e2) => {
                          customChatFoldMode.value = e2.currentTarget.checked;
                        }
                      }
                    ),
u$2("label", { htmlFor: "customChatFoldMode", style: { color: customChatEnabled.value ? void 0 : "#999" }, children: "去重折叠（合并 9 秒内的重复弹幕，显示 ×N）" })
                  ] }),
u$2("span", { className: "cb-switch-row", style: { display: "inline-flex", alignItems: "center", gap: ".25em" }, children: [
u$2(
                      "input",
...
u$2(
                      "input",
                      {
                        ...
                        onInput: (e2) => {
                          nativeChatFoldMode.value = e2.currentTarget.checked;
                        }
                      }
                    ),
u$2("label", { htmlFor: "nativeChatFoldMode", children: "B 站原生聊天框去重折叠（合并 9 秒内的重复弹幕，显示 ×N）" })
                  ] }),
u$2("div", { className: "cb-note", style: { paddingLeft: "1.5em", marginTop: ".25em" }, children: '和 Chatterbox Chat 那侧的"去重折叠"互不影响：这条改的是 B 站原生右侧聊天列表。' })
                ]
              }
            )
          ]
...
u$2("label", { htmlFor: "unlockSpaceBlock", children: "空间拉黑解锁（刷新生效，仅布局解锁）" })
  ...
  function validateGuardRoomEndpoint(raw) {
    let url;
    try {
      url = new URL(raw);
    } catch {
...
u$2(
              ...
              (() => {
                const v2 = guardRoomEndpoint.value.trim();
                if (v2 === "") return null;
                const warn = validateGuardRoomEndpoint(v2);
                if (!warn) return null;
                return u$2("span", { role: "status", "aria-live": "polite", style: { color: "#a15c00", fontSize: "0.8em" }, children: [
                  "⚠️ ",
                  warn
...
u$2("br", {}),
  ...
  const SECTION_KEYWORDS$1 = "radar 雷达 跨房间 meme 上报 report 观察 trending 隐私";
  function RadarSection({ query = "" }) {
    if (!matchesSearchQuery(SECTION_KEYWORDS$1, query)) return null;
    return u$2("details", { className: "cb-settings-accordion", children: [
...
🔵 云端测试完成！共测试 ${total} 个词，其中 ${totalBlocked} 个原词被屏蔽`);
    ...
    const didInit = A$1(false);
    y$2(() => {
      if (didInit.current) return void 0;
...
u$2(LayoutSection, { query }),
      matchesSearchQuery("表情 emote emoji 表情包 ID 复制 表情ID 表情ids", query) && u$2("details", { className: "cb-settings-accordion", open: true, children: [
...
u$2(GroupHeading, { query, children: "LLM（智驾选梗 + YOLO 润色共用）·「YOLO」= LLM 在你发出去之前先重写一遍" }),
u$2(LlmApiSection, { query }),
u$2(LlmPromptsSection, { query }),
...
u$2(BackupSection, { query }),
      matchesSearchQuery("日志设置 日志 行数 调试 debug log lines", query) && u$2("details", { className: "cb-settings-accordion", children: [
...
u$2("div", { className: "cb-note", style: { color: "#666" }, children: "收到「请发完整日志」类的反馈请求时打开此开关，再复制日志面板内容提交。" })
  ...
  function LlmApiSection({ query }) {
    const KEYWORDS = "llm api key model 模型 anthropic openai deepseek moonshot openrouter ollama 智能辅助驾驶 智驾 yolo 润色 base url 凭证 token 选梗";
    if (!matchesSearchQuery(KEYWORDS, query)) return null;
    return u$2("details", { className: "cb-settings-accordion", open: true, children: [
u$2("summary", { children: "LLM API 配置（智驾选梗 + YOLO 润色共用）" }),
u$2("div", { className: "cb-section cb-stack", style: { margin: ".5em 0", paddingBottom: "1em", gap: ".75em" }, children: [
u$2("div", { className: "cb-note", style: { color: "#666", fontSize: "0.85em" }, children: "填一次，「智能辅助驾驶」选梗 与「自动跟车 / 独轮车 / 常规发送」的 YOLO 润色都能用。" }),
u$2(
          "div",
          {
            className: "cb-note",
            ...
            children: [
u$2("strong", { children: "名词解释 ·「YOLO」" }),
              ' —— You Only Live Once，本项目里指"发出去之前先让 LLM 重写一遍弹幕"的开关。代价：每条弹幕都会调用一次大模型 API，产生 token 消耗；好处：可以套你写的提示词（卖萌/化身VTuber/避敏感词等）。 关闭后弹幕原样发出。'
            ]
          }
        ),
u$2(LlmApiConfigPanel, { showTestConnection: true })
      ] })
    ] });
  }
  function LlmPromptsSection({ query }) {
    const KEYWORDS = "llm 提示词 prompt yolo 润色 ai openai anthropic 全局基线 常规发送 自动跟车 独轮车 system prompt";
    if (!matchesSearchQuery(KEYWORDS, query)) return null;
    return u$2("details", { className: "cb-settings-accordion", children: [
u$2("summary", { children: "LLM 提示词（YOLO 用）" }),
u$2("div", { className: "cb-section cb-stack", style: { margin: ".5em 0", paddingBottom: "1em", gap: ".75em" }, children: [
u$2("div", { className: "cb-note", style: { color: "#666", fontSize: "0.85em" }, children: "这里只管理 YOLO 用的提示词。API 凭证（key / 模型 / base URL）在上面的「LLM API 配置」section 里填一次。" }),
u$2("div", { children: [
u$2("div", { className: "cb-heading", style: { fontWeight: "bold", marginBottom: ".25em" }, children: "全局基线" }),
u$2("div", { className: "cb-note", style: { color: "#666", fontSize: "0.85em", marginBottom: ".4em" }, children: '会作为通用前缀拼到下面三个功能特定提示词的前面（用 ↓ 双换行 + "以下是用户的修改提示" 分隔）。' }),
u$2(
            PromptManager,
            {
              ...
              onPromptsChange: (p2) => {
                llmPromptsGlobal.value = p2;
              },
              onActiveIndexChange: (i2) => {
                llmActivePromptGlobal.value = i2;
              },
...
u$2("div", { children: [
u$2("div", { className: "cb-heading", style: { fontWeight: "bold", marginBottom: ".25em" }, children: "常规发送" }),
u$2("div", { className: "cb-note", style: { color: "#666", fontSize: "0.85em", marginBottom: ".4em" }, children: "手动输入框 / 偷 / +1 等手动发送场景的修改提示。空 = 跳过 LLM。" }),
u$2(
            PromptManager,
            {
              prompts: llmPromptsNormalSend.value,
...
u$2("div", { children: [
u$2("div", { className: "cb-heading", style: { fontWeight: "bold", marginBottom: ".25em" }, children: "自动跟车" }),
u$2("div", { className: "cb-note", style: { color: "#666", fontSize: "0.85em", marginBottom: ".4em" }, children: "触发跟车后，把命中的弹幕用 LLM 润色一遍再发的修改提示。" }),
u$2(
            PromptManager,
            {
              prompts: llmPromptsAutoBlend.value,
...
u$2("div", { children: [
u$2("div", { className: "cb-heading", style: { fontWeight: "bold", marginBottom: ".25em" }, children: "独轮车" }),
u$2("div", { className: "cb-note", style: { color: "#666", fontSize: "0.85em", marginBottom: ".4em" }, children: "循环里每条非表情消息发送前用 LLM 润色的修改提示。配置不全会自动停车。" }),
u$2(
            PromptManager,
            {
              prompts: llmPromptsAutoSend.value,
  ...
  function SttTab() {
    ...
    const enumerateMics = async () => {
      ...
      try {
        ...
        audioDevices.value = devices2.filter((d2) => d2.kind === "audioinput");
      } catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
    ...
    const toggle = () => {
      if (state.value === "stopped") {
        ...
        statusColor.value = "#666";
        try {
          const client2 = new speechToTextWeb.SonioxClient({ apiKey });
          ...
          const savedDeviceId2 = sonioxAudioDeviceId.value;
          const deviceStillAvailable = !savedDeviceId2 || audioDevices.value.some((d2) => d2.deviceId === savedDeviceId2);
          if (savedDeviceId2 && !deviceStillAvailable) {
            appendLog("⚠️ 已选麦克风不可用，已切换至系统默认");
            sonioxAudioDeviceId.value = "";
          }
          const effectiveDeviceId = deviceStillAvailable ? savedDeviceId2 : "";
          const startConfig = {
            model: "stt-rt-v3",
    ...
    const hasMicLabels = devices.some((d2) => d2.label);
    const savedDeviceId = sonioxAudioDeviceId.value;
    const savedDeviceMissing = Boolean(savedDeviceId) && !devices.some((d2) => d2.deviceId === savedDeviceId);
    return u$2(S$1, { children: [
u$2(
        "div",
        {
...
u$2(
              "div",
              {
                ...
                style: { display: "flex", gap: ".5em", alignItems: "center", flexWrap: "wrap", marginBottom: ".5em" },
                children: [
u$2("label", { htmlFor: "sonioxAudioDevice", children: "设备：" }),
u$2("div", { style: { flex: "1 1 0", minWidth: 0, overflow: "hidden", display: "flex" }, children: u$2(
                    "select",
                    {
                      ...
                      children: [
u$2("option", { value: "", children: "系统默认" }),
                        devices.map((d2, i2) => u$2("option", { value: d2.deviceId, children: d2.label || `麦克风 ${i2 + 1}` }, d2.deviceId || `mic-${i2}`)),
                        savedDeviceMissing && u$2("option", { value: savedDeviceId, children: "(已保存设备不可用)" })
                      ]
                    }
                  ) }),
                  !hasMicLabels && u$2("button", { type: "button", onClick: () => void requestMicPermission(), style: { whiteSpace: "nowrap" }, children: "授权" }),
u$2("button", { type: "button", onClick: () => void enumerateMics(), style: { whiteSpace: "nowrap" }, children: "刷新" })
                ]
              }
            ),
u$2(
...
u$2(
                    "input",
                    {
                      ...
                      onInput: (e2) => {
                        const raw = Number.parseInt(e2.currentTarget.value, 10);
                        const v2 = Number.isFinite(raw) ? raw : 1;
                        sonioxMaxLength.value = Math.min(200, Math.max(1, v2));
                      }
                    }
                  ),
u$2("span", { children: "字自动分段" })
...
u$2("div", { className: "cb-floating-soft", style: { marginBottom: "8px", fontSize: "12px" }, children: "弹幕助手可以循环发送弹幕、按热度自动跟车、接管聊天区，并查询粉丝牌房间状态。" }),
...
u$2("div", { className: "cb-floating-softer", style: { fontSize: "12px" }, children: step.detail })
          ] }, step.title)) }),
u$2("div", { className: "cb-floating-softer", style: { fontSize: "12px", marginBottom: "8px" }, children: "部分功能（AI 规避、保安室同步、同传）会和外部服务通信，详见「关于 → 隐私说明」。" }),
  ...
  function CandidateRow({ candidate }) {
    return u$2(
      "div",
      {
        ...
        children: [
u$2("span", { className: "cb-floating-chip-meta", style: { minWidth: "4em", fontSize: "11px" }, children: candidate.label }),
u$2(
            "code",
            {
...
u$2(
  ...
  function ShadowBypassChip() {
    ...
    const dismiss = () => {
      ...
      const next = [...shadowChipDismissedKeys.value.filter((existing) => existing !== k2), k2];
...
u$2("div", { className: "cb-floating-divider", style: { fontSize: "11px", marginBottom: "4px" }, children: "候选改写（不自动发送，点「填入」会替换你的输入框文本）：" }),
