...
- **修复：自动跟车 / 独轮车 / WS / 队列等核心 send path 一次大规模审计加固** — 这一版没有新功能，主要是一轮覆盖 20+ 处的正确性 + 安全审计。挑几个用户能感觉到的：(1) `auto-blend triggerSend`：如果一波刷屏的所有 target 都被 locked emoticon / 不可用 / YOLO LLM 卡顿等过滤干净，**不再白白吃 20–45s 冷却**——冷却 + `trendMap.delete` 推迟到至少一条 target 通过所有过滤之后才落；burst 中途用户 toggle off 立刻停发（外层 ti 循环 + 内层 repeat 循环每轮都重检 `autoBlendEnabled`）；`lastAutoSentText` 只在 verified-success 时写入，cancelled / failed 不再污染「不重复上次自动发送」的判定。(2) 自动跟车 weighted random 数学边界 fallback 改成"按 count 排序后取最热门候选"，不再随机落到 map 迭代尾部那一条冷僻的。(3) 自动跟车 UID 黑名单切到 `Object.hasOwn(...)`——之前 UID 字符串恰好等于 `toString` / `constructor` 等原型链 key 时会被错误屏蔽，跟 2.13.2 文本黑名单那条修复对齐。(4) `live-ws-source`：`visibilitychange` 监听只在有 consumer 时挂、`startLiveWsSource` 完全 stop 时摘掉（之前一直泄漏在 module scope 跟 manual stop 抢逻辑）；live/close 事件用 `connectionSerial` 隔离，旧 socket 的回调不会把当前 socket 的 `reconnectAttempt` 清零；`addressIndex` 在一次成功 `'live'` 后归零，失败的多 endpoint 轮询不会跨次累积。(5) `send-queue`：硬上限 200 条，溢出时**只丢最老的 AUTO，MANUAL / STT 永远保留**，避免下游卡死时无限堆积。(6) `hzm-auto-drive` 每个 `await` 后重检 `hzmDriveEnabled`，LLM/启发式选梗 await 期间用户关掉智驾不会再多发一条。(7) `shadow-learn` cap-eviction 按 manual / learned 分区，**用户手工添加的规则永远不被淘汰**（之前的 `slice()` 写法跟文档"user wins"说明不符）；cloud sync 并发限到 3，不再把慢 Guard Room 端口打满。(8) `meme-contributor` 的 `times[]` / `nominationTimestamps` 按时间窗就地裁剪，不再无界增长。(9) `fetch-cache` 加 128 条 LRU 上限，长会话不再堆 URL key。(10) `radar-report` 的两条 signal effect 在 reset 时正确 dispose；房间切换 effect 只在真正发生 `prev !== curr` 切换时丢 buffer，订阅初次挂载不再误删。
- **修复：跨域请求 / LLM / AI 规避超时与日志脱敏** — `gm-fetch` 接受 `AbortSignal` 并在 abort 时主动 abort 底层 GM xhr；`llm-driver` 把 `ChatCompletionOptions.signal` 真正穿透下去（之前接受但忽略）；`ai-evasion.detectSensitiveWords` 加 10s `AbortController` 超时，LAPLACE 挂掉时不再无限挂起；`loop.ts` 里 wbi danmaku-config / fetchEmoticons 失败原本是空 catch，现在走 `appendLogQuiet` 留下诊断痕迹。HTTP 错误信息现在剥到 status + statusText（原始 upstream body 移到 `console.error`），错误日志里的 URL 砍到 origin+pathname——**用户自填的带 token 的 OpenAI-compat baseURL 不会再泄漏到 appendLog / notifyUser**。
- **修复：custom-chat 自定义 CSS 净化加强** — 之前的 `@import` / 危险 `url(...)` scheme / `expression(` / `behavior:` 黑名单可以用 CSS 转义绕过（比如 `@\69 mport`）。现在先 normalize CSS 转义、剥除注释，再跑黑名单匹配。
- **修复：Guard Room 接管参数校验** — `?guard_room_session` 现按 `[A-Za-z0-9_-]{1,128}` 校验；`guardRoomSyncKey` 没配时忽略 autostart / sessionId；可见接管会 `notifyUser('warning')` 提示；参数解析完用 `replaceState` 从 URL 抹掉，防止用户刷新页面重放接管。`guard-room-sync` 六个 endpoint 共享一个 8s timeout + `AbortController` 的 `fetchGuardRoom()` helper，`syncGuardRoomWatchlist` 也加上 try/catch（之前是 bare `fetch().then()`，HTTP / 网络错误会作为 `unhandledrejection` 逃逸）。
- **修复：`splitTextSmart` 对纯空白超长输入返回 `[]`** — 输入全是空白且 `length > maxLen` 时，函数循环顶部的"跳过 leading whitespace"会把所有 grapheme 吃光、返回空数组。下游 STT (`stt-tab`) 和智驾 (`hzm-auto-drive`) 都是 `for (const segment of segments)`，`[]` 直接 0 次迭代 → "啥也没发"。改为至少返回 `['']`，跟空字符串路径一致；加 ASCII 空格 / tab / 换行 / 全角空格四种回归。
- **修复：着陆页 `--blue` 颜色对比度不达 WCAG AA** — `#1677ff` 在浅色 `#f7f8fb` 上对比度 ~3.89，AA 4.5:1 不过。换成 Ant Design primary-darker 档的 `#0050b3`（白底对比 ~7.16 / 过 AAA，`.button.primary` 白字蓝底反过来同分）。暗色模式 `#66a8ff` 本来就过，保持。`.lighthouserc.json` 的 color-contrast 检查同时打回开，防回归。
- **修复（chatterbox-cloud 后端）：`POST /memes/bulk-mirror` IDOR + 攻击者输入污染** — (1) 请求里的 `uid` / `username` / `avatar` 一律强制 NULL；`external_id` / `copyCount` / `lastCopiedAt` / `createdAt` / `updatedAt` 全部按类型 + 范围校验后才入表。(2) tag-link **只挂到本批刚 insert 的 meme 上**——之前任何 `content_hash` 相同的 meme（包括已经过审的存量数据）都会被自动 link，构成 tag 涂鸦类 IDOR。`server/` 单测 64/64 通过。**自托管后端的 maintainer 注意：本版本 `server/` 有变更，`cd server && wrangler deploy` 才会生效；release workflow summary 也会贴一条醒目提醒。**
- **质量基建（用户无感）：完整接入 CodeQL + OSV-Scanner + Semgrep + Gitleaks + Knip + Lighthouse-CI + Stryker 突变测试 + size-limit PR 评论 + DeepSource lcov 上报** — 7 套静态 / 动态安全 + 代码质量门禁全部上线，所有第三方 GitHub Action 按 SHA pin + `# vX.Y.Z` 注释，findings 进 GitHub Security tab。dependabot 低风险 PR 自动 merge。低覆盖 11 个模块补到 90%+，新增 fast-check fuzz + 内存绑定 soak + 后端 autocannon 压测，突变测试 sharding 6 → 8、超时 60 → 120 min。打包产物 / 运行时行为零变化。
