...
- **接入 live-meme-radar 传感器（默认全部关闭）**：新增独立的 [live-meme-radar](https://live-meme-radar.pages.dev) 客户端模块（`src/lib/radar-client.ts`），暴露 `queryClusterRank` / `fetchTodayRadar` / `fetchTopAmplifiers` / `reportRadarObservation` 四个函数；雷达本身是只读、聚合、跨房间 meme 聚类的 Cloudflare Worker，不发送弹幕。
- **自动跟车软门**：设置 → 工具 → "Meme 雷达" 里新增 `radarConsultEnabled`。打开后，在 `triggerSend` 收齐目标但还没冷却之前会调一次 `/radar/cluster-rank`：雷达确认 trending → 打 boost log 后正常发；雷达明确 not-trending → 跳过本次（不冷却,trendMap 不消耗,等下一波）；雷达无匹配 / 网络挂 / 4s 超时 → 完全按本地逻辑继续,绝不阻塞主流程。
- **双源数据贡献开关**：新增 `radarReportEnabled`（也默认关闭）。`POST /radar/report` 在雷达 Week 9-10 才上线,在那之前打开开关也是无害的（静默 404）。客户端只送房间号 + channelUid + 时间窗口 + ≤30 条 dedupe 后的短文本（每条 ≤200 字）,从不送单条弹幕 + 发送者 uid 的对应关系。
- **新增设置区块和 @connect**：设置面板"工具"组下新增"Meme 雷达"折叠区，含两个开关 + 开发用 URL override。`@connect live-meme-radar.aijc-eric.workers.dev` 写进 userscript header（兜底的 `*` 仍在）。
- **测试覆盖**：新增 42 条 `radar-client.test.ts`，覆盖 URL 归一、cluster-rank 各种 matched/error 分支、列表/amplifier shape filter、report 的 dedupe + 静默失败,沿用项目既有的 `_setGmXhrForTests` DI seam,不动现有 1051 条测试。
